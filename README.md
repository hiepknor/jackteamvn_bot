...
- Thumbnail ảnh tùy chọn cho từng sản phẩm
...
- Kiểm soát truy cập bằng allowlist Telegram
...
- `handlers/command_handlers/`: tách command theo nhóm (general, data actions, product flows)
...
- `storage/thumbnails/`: ảnh thumbnail đã resize dạng JPEG

## 4) Cách tạo bot với BotFather
1. Mở Telegram, chat với `@BotFather`.
2. Dùng `/newbot` và làm theo hướng dẫn.
3. Lấy token và gán vào `TELEGRAM_BOT_TOKEN` trong `.env`.
...
Deploy production lên VPS mặc định:
```bash
make deploy
```
...
- `/edit` - Sửa sản phẩm
- `/delete` - Xóa sản phẩm
- `/thumbnail <id>` - Gán/thay ảnh thumbnail cho sản phẩm
- `/export` - Xuất dữ liệu
- `/stats` - Thống kê
...
- Thumbnail được lưu local trong `storage/thumbnails/`; khi xóa sản phẩm, file thumbnail tương ứng cũng bị xóa
